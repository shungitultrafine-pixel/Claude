...
## Texture confirmed (`home-material-crystal.png` inspected directly)

- RGBA, 1302×1208 (aspect ≈1.078), alpha channel present and clean —
  composited onto a dark background, no white fringe/halo at the edges.
  `sampleSilhouetteAnchors`'s alpha-based edge detection and the shader's
  `discard` cutout both work correctly against this file as-is.
- **The texture already has debris baked into its own silhouette** — roughly
  15-20 separate rock fragments scattered across nearly the full canvas
  (confirmed visually, grid-checked), not just the central dense cluster.
  This means `FragmentField`'s procedural particles were genuinely at risk
  of double-layering with the source image, not just hypothetically — cut
  `FAR_FRAGMENT_COUNT` 16→6, `NEAR_FRAGMENT_COUNT` 11→8, and pulled in
  `scatterMax`/`opacityMax` on both bands so the procedural fragments read
  as a few things *drifting near* the baked field rather than extending or
  duplicating it. If it still reads as too busy in-browser, cut further —
  these are still a judgment call on top of confirmed evidence, not a
  measurement of "correct" density.
- Full alpha content (dense cluster + all baked debris) spans roughly
  x:0-100%, y:2-100% of the texture's own canvas — i.e. the file was
  exported nearly edge-to-edge, very little empty margin. Worth knowing if
  `useContainScale`'s 80%-of-viewport fit ever looks tighter/looser than
  expected — there's very little "dead space" in the source to absorb.

## Needs your eyes (still can't verify without a live render)

1. **Alpha cutoff exact values** — `PLANE_ALPHA_DISCARD` (0.03),
   `SILHOUETTE_ALPHA_MIN` (140), `SILHOUETTE_NEIGHBOR_MAX` (60) — the file's
   alpha channel is confirmed clean, so these probably don't need much
   adjustment, but "probably" isn't "confirmed in-browser".
2. **Object position/scale, live** — insets from Position Calibration above
   were measured off the composition reference, not this texture; confirm
   they still land correctly now that the real aspect ratio (1.078) is in
   play.
3. **Light intensity** — key/fill reduced to 0.22/0.08, particle warm/cool
   narrowed to 1.04/0.93 — reasoned adjustments given the texture's own
   fairly warm/amber baked rim lighting (visible in the file), not measured
